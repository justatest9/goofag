...
for ((i = 1; i <= $TOTAL_CHUNKS; i++)); do
  grep reality "$TMP/ebrasha$i.txt" | python $PTH/remove_duplicates.py | bash $PTH/mttest/test.sh >>$TMP/TESTED_EBA.txt
done
...
echo $PTH
cat $TMP/TESTED_EBA.txt | sort -r -n | cut -d \  -f 2- | python $PTH/remove_duplicates.py >$TMP/TESTED_EBA_CUT.txt
